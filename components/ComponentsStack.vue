...
async function initChart() {
  try {
    ...
    const dates = data.calendar.ds.filter((_, i) => i % 14 === 0)
    const weekly = data.ground_truth.weekly.filter((_, i) => i % 14 === 0)
    const yearly = data.ground_truth.yearly.filter((_, i) => i % 14 === 0)
    const holiday = data.ground_truth.holiday.filter((_, i) => i % 14 === 0)
    const competitor = data.ground_truth.competitor.filter((_, i) => i % 14 === 0)
    const promo = data.ground_truth.promo.filter((_, i) => i % 14 === 0)
    ...
    option.value = {
      ...
      legend: {
        data: ['Weekly', 'Yearly', 'Holidays', 'Competitor', 'Promos'],
      ...
      series: [
        ...
        { name: 'Promos', type: 'line', data: promo, smooth: true, lineStyle: { width: 2, color: '#f59e0b' }, itemStyle: { color: '#f59e0b' }, showSymbol: false, emphasis: { focus: 'series' } }
      ]
    }
  } catch (error) {
